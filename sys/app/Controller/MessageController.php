...
//        $pgnum = Hash::get($this->request->query, 'pg');
        ...
        $arrToMessage = $this->Message->getPagingEntity($disp_num,$pgnum, 'receive');
        ...
        $param['status'] = 1;
        $this->Message->setWhereVal($param);
        $arrFromMessage = $this->Message->getPagingEntity($disp_num,$pgnum, 'send');
    ...
    function new_message($to_member_id, $from_item_id=null){
        ...
        if ($this->request->is('post') or $this->request->is('put')) {
            ...
            if($this->Message->isSend($data)){
                ...
                // 販売者にメールを送信
                if($data['Message']['status'] == 1 && $arrToMember['Member']['notification_when_dm_flag'] == 1){
                    ...
                    $viewVars['value']['company'] = $arrMember['company'];
                    $viewVars['value']['nickname'] = $arrMember['nickname'];
                    $viewVars['value']['body'] = $data['Message']['body'];
                    $viewVars['value']['subject'] = $data['Message']['subject'];

                    // メール送信
                    $user_id = $arrToMember['Member']['user_id'];
                    $this->sendMail($messageToSeller, $user_id, $viewVars);
                }
                // ●商品問い合わせ時メール送信 2017/12/01 add --END
 ...
 * **/
    ...
    function sendinglist(){
        ...
        $param['status'] = 1;
        $this->Message->setWhereVal($param);
        $arrMessage = $this->Message->getPagingEntity($disp_num,$pgnum, 'send');
    ...
    function receivedlist(){
        ...
        $param['status'] = 1;
        $this->Message->setWhereVal($param);
        $arrMessage = $this->Message->getPagingEntity($disp_num,$pgnum, 'receive');
        $this->set('arrMessage', $arrMessage);
    ...
    function message_detail($id){
		...
		$arrListData = $this->Message->getList($f_m_id, $t_m_id, $p_id, $this->user_id);
        // 既読処理
